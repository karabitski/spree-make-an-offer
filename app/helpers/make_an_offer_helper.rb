...
module MakeAnOfferHelper
  def offer_status(offer)
    ...
    when offer.counter_price.present? && offer.counter_accepted.nil?
      content_tag(:span, t('offer.counter_offered', date: offer.updated_at.strftime('%d/%m/%y às %H:%m')), class: 'state pending')
    when offer.counter_price.present? && offer.counter_accepted.present?
      content_tag(:span, t('offer.counter_offer_accepted', date: offer.counter_accepted.strftime('%d/%m/%y às %H:%m')), class: 'state complete')
    else
